...
if __name__=="__main__":
    ...
    parser.add_argument('--eeg_dataset',
                        type=str,
                        default="./data/eeg/theperils/FixedSampling/spampinato-1-IMAGE_BLOCK_RAW_with_mean_std.pth",
                        help='Dataset to train')
    # parser.add_argument('--eeg_dataset',
    #                     type=str,
    #                     default="./data/eeg/theperils/FixedSampling/spampinato-1-IMAGE_RAPID_RAW_with_mean_std.pth",
    #                     help='Dataset to train')
    # parser.add_argument('--eeg_dataset',
    #                     type=str,
    ...
    parser.add_argument('--custom_model_weights',
                        type=str,
                        default="./logs/FixedSamples/withnorm/spampinato-1-IMAGE_RAPID_RAW_with_mean_std_experimental_given_blockimageSeqToRapid\lstm_dinov2_best_loss.pth",
    ...
    model = LSTMModel(input_size=LSTM_INPUT_FEATURES,hidden_size=LSTM_HIDDEN_SIZE, out_features=384, n_layers=4)
    print(model)
    ...
    query_features = []
    query_labels = []
    for eeg, label, image, idx, img_f in dataset:
        gallery_features.append(img_f)
        gallery_labels.append(label)
    
    for eeg, label, image, idx, img_f in test_dataset:
        query_features.append(img_f)
    ...
    for query_idx, search_res in enumerate(I):
        ...
        test_intlabel = test_label["ClassId"]
        test_strlabel = test_label["ClassName"]
        ...
        for search_res_idx in search_res:
            s_label  = gallery_labels[search_res_idx]
            intlabel = s_label["ClassId"]
            strLabel = s_label["ClassName"]
