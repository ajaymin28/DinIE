...
class FeatureDistributionLoss(nn.Module):
    ...
    def forward(self, student_outputs, teacher_outputs, epoch, label, pred_label=None):
        ...
        ce_loss = nn.functional.cross_entropy(pred_label, label)
...
if __name__=="__main__":
    ...
    parser.add_argument('--log_dir',
                        type=str,
                        default='./logs/DINOvsDinIE/classLoss/',
    ...
    if os.path.exists(f"{FLAGS.log_dir}/spampinato_lstm_dinov2_best_loss.pth"):
        print(f"Path exist, already, either rename or delete the previous model, {FLAGS.log_dir}/spampinato_lstm_dinov2_best_loss.pth")
        exit()
    ...
    data_loader_val = DataLoader(val_dataset, batch_size=FLAGS.batch_size, shuffle=False)
    ...
    for EPOCH in range(EPOCHS):
        ...
        if best_val_loss is None:
            ...
            if val_epoch_loss<best_val_loss:
                ...
                torch.save(model.state_dict(), f"{FLAGS.log_dir}/spampinato_lstm_dinov2_best_loss.pth")
    ...
    }, f"{FLAGS.log_dir}/spampinato_lstm_dinov2_best_loss.pth")
